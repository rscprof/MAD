...
func main() {
	http.HandleFunc("/restaurants", getRestaurantsByCity)
...
// Функция для поиска ресторанов по месту
func getRestaurantsByCity(w http.ResponseWriter, r *http.Request) {
	...
	if len(result) == 0 {
		http.Error(w, "No restaurants found for the specified city", http.StatusNotFound)
		return
	}
