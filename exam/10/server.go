...
type Book struct {
	ID     string `json:"id"`     // Уникальный идентификатор книги
...
// Функция поиска книг по диапазону годов издания
func searchBooksByYearRange(w http.ResponseWriter, r *http.Request) {
	...
	// Если параметры некорректны, вернуть ошибку
	if err1 != nil || err2 != nil {
		http.Error(w, "Invalid year range parameters", http.StatusBadRequest)
		return
	}
...
func main() {
	http.HandleFunc("/search", searchBooksByYearRange)
