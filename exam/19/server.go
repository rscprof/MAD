...
var apartments = []Apartment{
	...
	{"77", "Dallas", 600000, 100, 11, 4, 2021},
	{"78", "Chicago", 500000, 85, 10, 3, 2019},
...
func main() {
	http.HandleFunc("/apartments", getApartmentsByPrice)
...
// Функция для поиска квартир по цене
func getApartmentsByPrice(w http.ResponseWriter, r *http.Request) {
	minPrice := r.URL.Query().Get("min_price")
	maxPrice := r.URL.Query().Get("max_price")

	if minPrice == "" || maxPrice == "" {
		http.Error(w, "Both min_price and max_price query parameters are required", http.StatusBadRequest)
		return
	}
	...
	if len(result) == 0 {
		http.Error(w, "No apartments found within the specified price range", http.StatusNotFound)
		return
	}
