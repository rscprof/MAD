...
func searchPointsByLocation(w http.ResponseWriter, r *http.Request) {
	latQuery := r.URL.Query().Get("latitude")
	lonQuery := r.URL.Query().Get("longitude")
	radiusQuery := r.URL.Query().Get("radius")

	if latQuery == "" || lonQuery == "" || radiusQuery == "" {
		http.Error(w, "Parameters 'latitude', 'longitude', and 'radius' are required", http.StatusBadRequest)
		return
	}
	...
	longitude, err := strconv.ParseFloat(lonQuery, 64)
	if err != nil {
	...
	matchingPoints := []PointOfInterest{}
	for _, poi := range pointsOfInterest {
		distance := haversineDistance(latitude, longitude, poi.Latitude, poi.Longitude)
		if distance <= radius {
			matchingPoints = append(matchingPoints, poi)
		}
	}
	...
	if len(matchingPoints) == 0 {
		...
		json.NewEncoder(w).Encode(map[string]string{"error": "No points of interest found within the specified radius"})
		return
	}

	json.NewEncoder(w).Encode(matchingPoints)
}

func haversineDistance(lat1, lon1, lat2, lon2 float64) float64 {
	const earthRadiusKm = 6371.0 // Радиус Земли в километрах
	lat1Rad, lon1Rad := degreesToRadians(lat1), degreesToRadians(lon1)
	lat2Rad, lon2Rad := degreesToRadians(lat2), degreesToRadians(lon2)
...
func main() {
	http.HandleFunc("/pois/search", searchPointsByLocation)
