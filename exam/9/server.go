...
// Place представляет туристическое место
// @Description Модель для представления туристических мест
// @type Place
type Place struct {
	ID          string `json:"id"`          // Уникальный идентификатор места
	Name        string `json:"name"`        // Название места
	Country     string `json:"country"`     // Страна, где расположено место
	Description string `json:"description"` // Описание места
	Popularity  int    `json:"popularity"`  // Оценка популярности (0-100)
}
...
// Функция поиска туристических мест по диапазону популярности.
// @Summary Поиск туристических мест по диапазону рейтинга популярности
// @Description Функция позволяет найти места по диапазону рейтинга популярности, используя параметры 'min' и 'max' в запросе.
// @Tags search
// @Param min query int true "Минимальный рейтинг популярности"
// @Param max query int true "Максимальный рейтинг популярности"
// @Success 200 {array} Place "Список мест"
// @Failure 400 {string} string "Неверный запрос"
// @Failure 500 {string} string "Ошибка на сервере"
// @Router /searchByPopularity [get]
func searchPlacesByPopularity(w http.ResponseWriter, r *http.Request) {
	// Получаем параметры min и max из запроса
	...
	// Отправляем результаты в виде JSON
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(results)
}

func main() {
	// Роутинг и запуск сервера
	http.HandleFunc("/searchByPopularity", searchPlacesByPopularity)
