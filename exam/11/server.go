...
func searchBooksByTitle(w http.ResponseWriter, r *http.Request) {
	...
	if titleQuery == "" {
		http.Error(w, "Parameter 'title' is required", http.StatusBadRequest)
		return
	}

	matchingBooks := []Book{}
	for _, book := range matchingBooks {
		if strings.Contains(strings.ToLower(book.Title), strings.ToLower(titleQuery)) {
	...
	if len(matchingBooks) == 0 {
		...
		json.NewEncoder(w).Encode(map[string]string{"error": "No books found matching the title"})
		return
	}
...
func main() {
	http.HandleFunc("/books/search", searchBooksByTitle)
