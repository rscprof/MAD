...
// Функция для обработки запроса поиска по категории
func searchByCategory(w http.ResponseWriter, r *http.Request) {
	// Получаем параметр "category" из URL-запроса
	...
	if len(filteredLocations) == 0 {
		http.Error(w, "No locations found for this category", http.StatusNotFound)
		return
	}
