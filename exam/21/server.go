...
func main() {
	http.HandleFunc("/apartments", getApartmentsByRooms)
...
// Функция для поиска квартир по количеству комнат
func getApartmentsByRooms(w http.ResponseWriter, r *http.Request) {
	minRooms := r.URL.Query().Get("min_rooms")
	maxRooms := r.URL.Query().Get("max_rooms")

	if minRooms == "" || maxRooms == "" {
		http.Error(w, "Both min_rooms and max_rooms query parameters are required", http.StatusBadRequest)
		return
	}

	// Преобразуем строковые значения в int
	minRoomsVal, err := strconv.Atoi(minRooms)
	if err != nil {
		http.Error(w, "Invalid min_rooms value", http.StatusBadRequest)
		return
	}

	maxRoomsVal, err := strconv.Atoi(maxRooms)
	if err != nil {
		http.Error(w, "Invalid max_rooms value", http.StatusBadRequest)
		return
	}

	// Ищем квартиры в указанном интервале количества комнат
	var result []Apartment
	for _, apartment := range apartments {
		if apartment.Rooms >= minRoomsVal && apartment.Rooms <= maxRoomsVal {
	...
	if len(result) == 0 {
		http.Error(w, "No apartments found within the specified room range", http.StatusNotFound)
		return
	}
