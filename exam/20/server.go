...
func main() {
	http.HandleFunc("/apartments", getApartmentsByYear)
...
// Функция для поиска квартир по году постройки
func getApartmentsByYear(w http.ResponseWriter, r *http.Request) {
	minYear := r.URL.Query().Get("min_year")
	maxYear := r.URL.Query().Get("max_year")

	if minYear == "" || maxYear == "" {
		http.Error(w, "Both min_year and max_year query parameters are required", http.StatusBadRequest)
		return
	}

	// Преобразуем строковые значения в int
	minYearVal, err := strconv.Atoi(minYear)
	if err != nil {
		http.Error(w, "Invalid min_year value", http.StatusBadRequest)
		return
	}

	maxYearVal, err := strconv.Atoi(maxYear)
	if err != nil {
		http.Error(w, "Invalid max_year value", http.StatusBadRequest)
		return
	}
	...
	if len(result) == 0 {
		http.Error(w, "No apartments found within the specified year range", http.StatusNotFound)
		return
	}
